    ...
    //    FormLineList fll;
        ...
        //    fll = FLL;
         ...
         //   fll.Close();
            ...
            comboBox1.Items.Add("ГП");
            comboBox1.Items.Add("СПЕЦИИ");
         //   comboBox1.Items.Add("Отходы");
            comboBox1.Items.Add("СЫРМАТ");
         ...
         /*   for (int i = PlanDataList.Count - 1; i >= 0; i--)
            ...
            var NonZero = fm.tdb.pr_GetAlarmWaste().Where(x => (x.WorkDate.Date == DateTime.Today.Date) && (x.WorkDate.Hour >= 6) && (x.WorkDate.Hour <= 10)).ToList();
           ...
           //     sbpList8 = sbpList8.Where(x => fm.StorageNamesList.Contains(x.LineName)).ToList();
               ...
               /* if (sbListIn.Count>0)
                ...
                ml.RawStorage = sbListMes8.Sum(x => (double)x.Quantity * -1) -  sbListInBlock.Sum(x => (double)x.Quantity);  /*- sbListOutBlock.Sum(x => x.Quantity)*/   // sbList8.Sum(x => x.Quantity) - ...
                ...
                ml.RawStorageUD = sbListMes8.Where(x => x.BBFDate >= DateTime.Today.Date).Sum(x => (double)x.Quantity * -1) - sbListInBlock.Sum(x => (double)x.Quantity) /*- sbListOutBlock.Sum(x => x.Quantity)*/;   //sbList8.Where(x => x.BBFDate >= DateTime.Today.Date).Sum(x => x.Quantity)
                ...
                else
                {
                    ...
                    {  //и для складов
                        ...
                        if (ListAA.Count > 0)
                        {
                            ...
                            {
                                string DString = aa.ProdDate.ToString("dd.MM.yyyy") + @"/" + aa.BBFDate.ToString("dd.MM.yyyy");
                        ...
                        if (cfm1.Count > 0)
                        {
                            ...
                            if (EL.Count > 0)
                            {
                                ...
                                {
                                    string DString = el.ProdDate.ToString("dd.MM.yyyy") + @"/" + el.BBFDate.ToString("dd.MM.yyyy");
         ...
         //   if (cbPlanFilter.Checked == true)
            {
              //  ClearDGV();
                ...
                Cursor = Cursors.WaitCursor;
                if (comboBoxFiltering.Text.Trim() != "" && comboBoxFiltering.SelectedIndex >0)
                {
